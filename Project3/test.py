...
if twoplanets == 1:
    ...
    S[:,0] = np.reshape(np.array((x - 6e7, x + 6e7)), (particles*2)) # x for the two planets
    S[:,1] = np.reshape(np.array((y - 6e7, y + 6e7)), (particles*2))# y 
    S[:,2] = np.reshape(np.array((z, z)), (particles*2)) # z
    S[:,3] = np.reshape(np.array((vx + 1000, vx - 1000)), (particles*2)) # vx
    S[:,4] = np.reshape(np.array((vy + 1000, vy - 1000)), (particles*2)) # vy
    S[:,5] = np.reshape(np.array((vz, vz)), (particles*2)) # vz
    mass = np.reshape(np.array((initialcond[:,6], initialcond[:,6])), (particles*2))   # mass
...
if intvar == 1:
    steps = 1000
    S_int = RK45(integrate, 0, S, steps*3600, max_step = 25.0)#, rtol=0.001, atol=1e-6)
...
np.save('300planet-diffmass-headon{}.npy'.format(twoplanets), S_i)
...
S_i_300 = np.load('C:\\Users\\piotr\\Documents\\GitHub\\Computational-Astrophysics\\300planet-diffmass-headon{}.npy'.format(twoplanets))
...
plt.tight_layout()
plt.savefig('1200edgeon-1150{}.png'.format(twoplanets), dpi=300,  bbox_inches='tight')
